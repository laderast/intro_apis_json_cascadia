...
2. Receive a response from the API server
3. Figure out how to get the data out of the response (usually involves working with the JSON).
...
This data was originally encoded in JSON, but `fromJSON()` is smart enough to guess that it's a data.frame. That's because it was organized as a list, with each row encoded as a separate object.
...
However, there are a lot of JSON documents that aren't that friendly. That's where `{tidyjson}` comes in.
